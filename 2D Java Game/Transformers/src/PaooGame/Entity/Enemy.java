...
import PaooGame.Object.BlackFireBall;
...
public class Enemy extends  Entity{
    ...
    public void setDefaultValues()  {
        ...
        direction = "idle";
        lastDirection = "right";
        projectile = new BlackFireBall(gp);
    }

    public void setAction(){
        int dx = this.worldX - gp.player.worldX;
        int dy = this.worldY - gp.player.worldY;
        if(java.lang.Math.abs(dx) <= 256 && java.lang.Math.abs(dy) <= 256){
            if(dx <= 0){
                if(dy > 0) {
                    if(dx * (-1) > dy)
                        direction = "right";
    ...
    public void draw(Graphics2D g){
        ...
        if(worldX + 64 > gp.player.worldX - gp.player.screenX &&
            ...
            Animation.enemyAni(g, direction, lastDirection, shoot, x, y, spriteNum, invincible);
        }
